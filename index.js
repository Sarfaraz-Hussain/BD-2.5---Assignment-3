...
function sortByPricingHighToLow(h1, h2) {
  return h2.price - h1.price;
}

app.get('/hotels/sort/pricing', (req, res) => {
  let pricing = req.query.pricing;
  let myHotels = hotels.slice();
  if (pricing === 'low-to-high') {
    myHotels.sort(sortByPricingLowToHigh);
  }
  if (pricing === 'high-to-low') {
    myHotels.sort(sortByPricingHighToLow);
  }
  res.json(myHotels);
});
...
function sortHighToLow(h1, h2) {
  return h2.rating - h1.rating;
}
function sortLowToHigh(h1, h2) {
...
app.get('/hotels/sort/rating', (req, res) => {
  ...
  if (rating === 'high-to-low') {
    myHotels.sort(sortHighToLow);
  }
  if (rating === 'low-to-high') {
    myHotels.sort(sortLowToHigh);
  }
  res.json(myHotels);
});
...
function sortLeastToMost(h1, h2) {
  return h1.reviews - h2.reviews;
}
function sortMostToLeast(h1, h2) {
...
app.get('/hotels/sort/reviews', (req, res) => {
  ...
  if (reviews === 'least-to-most') {
    myHotels.sort(sortLeastToMost);
  }
  if (reviews === 'most-to-least') {
    myHotels.sort(sortMostToLeast);
  }
  res.json(myHotels);
});
